drop database if exists miniprj;
...
-- Thống kê giá trung bình, thấp nhất và cao nhất của sản phẩm theo danh mục: . Nguyễn Tuấn Minh
...
-- Lấy danh sách sản phẩm có giá cao hơn giá trung bình của tất cả sản phẩm: khánh
select 
    product_id,
    product_name,
    price
...
-- Lấy danh sách khách hàng đã từng đặt ít nhất một đơn hàng: tiến
select *from customers
where customer_id in (
  select customer_id from orders 
);
-- Lấy đơn hàng có tổng số lượng sản phẩm lớn nhất tiến
select *from orders 
where order_id in (
  select max(quantity) from order_items
);
-- Lấy tên khách hàng đã mua sản phẩm thuộc danh mục có giá trung bình cao nhất: Phạm Quốc Lộc
...
-- Từ bảng tạm (subquery), thống kê tổng số lượng sản phẩm đã mua của từng khách hàng: Huy
...
-- Không gây lỗi “Subquery returns more than 1 row”: thắng
SELECT *
FROM products
WHERE price = (
    SELECT MAX(price)
